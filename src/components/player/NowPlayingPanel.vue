...
let _lowBaseline = 0                         // 自适应基线，缓慢跟踪 long-term 平均（类似 sidechain 压缩器）
...
function startRhythmLoop() {
  ...
  function step() {
    ...
    // 低频能量变化率 — 自适应基线 delta
    // 基线缓慢跟踪长期平均（因子 0.005，~2~3s 时间常数），只对"超出当前基线"的瞬间响应
    // 类似音频 sidechain 压缩器：无论歌有多吵（恒定高能量），鼓点瞬态仍可抓取
    ...
    _accumulator *= 0.97
    // scale = 基础 + delta脉冲 + 累积能量 + 中频能量（吉他/人声等持续乐器也能驱动膨胀）
    const sc = Math.round(Math.min(2.0, 1 + _deltaSmoothed * 8 + _accumulator * 3 + _rhythmEnergy.mid * 0.4) * 100) / 100
...
function stopRhythmLoop() {
  ...
  _lowBaseline = 0; _deltaSmoothed = 0; _accumulator = 0
  window.electronAPI?.rhythmUpdate({ lowRaw:0,midRaw:0,fullRaw:0, lowSmoothed:0,midSmoothed:0,fullSmoothed:0, lowBaseline:0,lowDelta:0,deltaSmoothed:0,accumulator:0, scale:'1',midOp:'0.7',hlOp:'0.55', flowElFound:false })
}
